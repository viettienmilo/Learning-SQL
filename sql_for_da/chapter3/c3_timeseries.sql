...
/* Seasonality
 Compare sales of same month of every year 
 (Result IN FILE c3_plot_7.csv)
 */
SELECT
    MONTHNAME(sales_month) AS `MONTH`,
    MAX(
        CASE
            WHEN YEAR(sales_month) = '2016' THEN sales
        END
    ) AS sales_2016,
    MAX(
        CASE
            WHEN YEAR(sales_month) = '2017' THEN sales
        END
    ) AS sales_2017,
    MAX(
        CASE
            WHEN YEAR(sales_month) = '2018' THEN sales
        END
    ) AS sales_2018,
    MAX(
        CASE
            WHEN YEAR(sales_month) = '2019' THEN sales
        END
    ) AS sales_2019,
    MAX(
        CASE
            WHEN YEAR(sales_month) = '2020' THEN sales
        END
    ) AS sales_2020
FROM
    retail_sales
